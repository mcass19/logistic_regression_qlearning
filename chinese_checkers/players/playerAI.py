...
class PlayerAI(Player):
    ...
    def load_weights(self):
        if self.id == 1:
            with open('chinese_checkers/players/weights_1.txt', 'r') as file_to_read:
            ...
            with open('chinese_checkers/players/weights_2.txt', 'r') as file_to_read:
                file_to_read.readline()  # Salta la primer linea (es un comentario)
                for i in range(1, 7):
                    # Agrega a la lista de pesos los pesos iniciales definidos en el archivo
                    self.weights.append(float(file_to_read.readline()))
    ...
    def save_weights(self):
        if self.id == 1:
            with open('chinese_checkers/players/weights_1.txt', 'w') as file_to_write:
                file_to_write.write('# Weights for p1\n')
                file_to_write.writelines([str(c) + '\n' for c in self.weights])
        else:
            with open('chinese_checkers/players/weights_2.txt', 'w') as file_to_write:
